...
public class InputBasedHoverMovement : MonoBehaviour, IPossessionSensitive, IPossessionSource, IDeathResponse,
    IPlayerRespawnListener
{
    ...
    [Header("Movement")] [SerializeField] private float _maxSpeed = 4f;
    ...
    void FixedUpdate()
    {
        ...
        CharacterMove(_inputSource.HorizontalMovementVector);
    }
    ...
    private void CharacterMove(Vector3 horizontalMoveInput)
    {
        ...
        float
            velDot = Vector3.Dot(_goalDirFromInput,
                unitDir); // checking difference in direction in current input and current velocity direction?
        ...
        //actual force
        Vector3 neededAccel =
            (_goalVel - _rb.linearVelocity) /
            Time.fixedDeltaTime; // acceleration needed to reach max accel in 1 fixed update?

        float maxAccel = _maxAccelForce * _maxAccelerationForceFactorFromDot.Evaluate(velDot) * _maxAccelForceFactor;
        neededAccel = Vector3.ClampMagnitude(neededAccel, maxAccel);
        _rb.AddForceAtPosition(Vector3.Scale(neededAccel * _rb.mass, _moveForceScale),
            _rb.position +
            new Vector3(0f, _rb.transform.localScale.y * _leanFactor,
