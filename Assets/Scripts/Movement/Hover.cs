...
public class Hover : MonoBehaviour, IPossessionSource, IDeathResponse, IPlayerRespawnListener
{
    ...
    [TextArea] public string Warning =
        "Please note that for now _uprightSpringDamper, _uprightSpringStrength and _rideSpringStrength require exiting play mode to change properly if changing them or the rigidbody's mass!";
    ...
    [Header("Height Spring Settings")] [SerializeField] [Min(0.1f)]
    private float _rideHeight = 0.93f;
    ...
    [Header("Upright Spring Settings")] [SerializeField] [Min(0.1f)]
    private float _uprightSpringDamper = 25f;
    ...
    [Header("Debug")] [SerializeField] private bool _showGroundRay;
    ...
    {
        Vector3 rayDir = _rb.transform.TransformDirection(_downDir);

        Ray rayToGround = new Ray(_rb.position, rayDir);
        _rayHitGround = Physics.Raycast(rayToGround, out _rayHit, _raycastToGroundLength, _groundLayer);

        if (_rayHitGround)
        {
            if (_rayHit.distance <=
                _rideHeight * 1.3f) // 1.3f? multiplied because object will oscillate but 1.3 is random
            {
                _isGrounded = true;
            }
    ...
    private void MaintainHeight()
    {
        if (_rayHitGround)
        {
            ...
            float rideSpringDamper =
                2f * Mathf.Sqrt(_rideSpringStrength * mass) * _rideSpringDampingRatio; //from zeta formula 

            float distanceFromRideHeight = _currentDistanceFromGround - _rideHeight;

            float springForce = (distanceFromRideHeight * _rideSpringStrength) - (relVel * rideSpringDamper);
    ...
    {
        DebugUtils.DrawLine(transform.position, transform.position + Vector3.down * _raycastToGroundLength,
            _debugRayThickness, Color.red);
    }
